...
DateRangeWidget
===============
Widget بتاريخين (من / إلى) بنفس شكل DateHeaderWidget
يُستخدم في Reports_screen.py بدل أزرار الفلتر الزمني

كيفية الاستخدام:
    self.date_range = DateRangeWidget()
    self.date_range.changed.connect(self.load_data)
    layout.addWidget(self.date_range)

    # جلب التاريخين:
    date_from, date_to = self.date_range.get_range()
"""
...
# ══════════════════════════════════════════
#  SingleDateCell — خلية تاريخ واحد (من / إلى)
# ══════════════════════════════════════════
...
# ══════════════════════════════════════════
#  DateRangeWidget — الـ widget الكامل
# ══════════════════════════════════════════
...
class DateRangeWidget(QWidget):
    """
    Widget بتاريخين (من / إلى) بنفس شكل DateHeaderWidget
    + زرار "الكل" يرجع الفترة لـ 30 يوم ماضية
    """
    ...
    def get_range(self) -> tuple[str, str]:
        """يرجع (date_from, date_to) بصيغة 'yyyy-MM-dd'"""
        d_from = self._from_cell.date()
        d_to = self._to_cell.date()
        # لو عكس الترتيب نصلحه تلقائياً
        if d_from > d_to:
